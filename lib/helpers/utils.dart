...
class Utils {
  ...
  static bool isMobile() {
    final width = MediaQuery.of(mainNav.currentContext!).size.width;
    return width < 600; // 600 px se chhota = mobile
  }

  static void showMessageOnSnack(String title, String message) {
    final theme = Theme.of(mainNav.currentContext!).snackBarTheme;
    final iconColor = theme.closeIconColor;
    final messageStyle = theme.contentTextStyle!.copyWith(
      fontWeight: FontWeight.normal,
    );

    ScaffoldMessenger.of(mainNav.currentContext!).showSnackBar(
      SnackBar(
        content: Column(
